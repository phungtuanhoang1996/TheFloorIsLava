...
public class GameRenderer {
    ...
    private void shop(float runTime) {
        ...
        batcher.draw(AssetLoader.slow_down, 1345, 813, 150, 150);
        batcher.draw(AssetLoader.coin, 1345, 983, 33, 33);
        AssetLoader.parisine_36_white.draw(batcher, "500", 1388, 983);
        AssetLoader.parisine_36_white.draw(batcher, String.format("x %d", prefs.getInteger("slow_down")), 1529, 863);
    ...
    public void running(float runTime) {
        ...
        if (grid.gem != null) {
            for (int i = 0; i < grid.gem.size(); i++) {
                if (grid.gem.get(i).isScored == true) {
                    if (!AssetLoader.exhaustAnimation.isAnimationFinished(runTime)) batcher.draw(AssetLoader.exhaustAnimation.getKeyFrame(runTime), grid.gem.get(i).getX(), grid.gem.get(i).getY());
                    continue;
                }
        ...
        if (grid2.gem != null) {
            for (int i = 0; i < grid2.gem.size(); i++) {
                if (grid2.gem.get(i).isScored == true) {
        ...
        */
        ...
        switch (prefs.getInteger("flag")) {
            case 1: {
                batcher.draw(AssetLoader.flag1Animation.getKeyFrame(runTime), player.getX() - 200, player.getY());
                break;
            }
            case 2: {
                batcher.draw(AssetLoader.flag2Animation.getKeyFrame(runTime), player.getX() - 200, player.getY());
                break;
            }
            case 3: {
                batcher.draw(AssetLoader.flag3Animation.getKeyFrame(runTime), player.getX() - 200, player.getY());
                break;
            }
        }
        ...
        if (myWorld.isReady()) {
            ...
            */
            ...
            if (player.isAlive()) {
                ...
                batcher.draw(AssetLoader.plus_time, 0, 341, 125, 125);
                batcher.draw(AssetLoader.slow_down, 0, 341 + 150 + 30, 125, 125);
                AssetLoader.parisine_36_white.draw(batcher, String.format("x %d", prefs.getInteger("plus_time")), 150, 385);
                AssetLoader.parisine_36_white.draw(batcher, String.format("x %d", prefs.getInteger("slow_down")), 150, 565);
            }
            // Draw text
